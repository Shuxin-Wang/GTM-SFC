...
class SFCBatchGenerator:
    ...
    def get_sfc_batch(self):
        ...
        for batch in range(self.batch_size):
            ...
            for i in range(self.sfc_length[batch]):
                vnf_type = random.randint(1, self.num_vnf_types)    # vnf type = 0 for empty
    ...
    @staticmethod
    def vnf_type_to_qos(vnf_type):
        if vnf_type > 0:
            return [VNF_SIZE[vnf_type], VNF_LATENCY[vnf_type], VNF_BANDWIDTH[vnf_type]]
        else:
            return [0, 0, 0]
